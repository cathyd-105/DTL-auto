...
class createPuVmPage(BasePage):
    ...
    public_button = (By.XPATH, '//span[text()="Public"]')
    ...
    pu_vm1 = (By.LINK_TEXT, 'ding-pu000')
    pu_vm2 = (By.LINK_TEXT, 'ding-pu001')
    ...
    def basic_set(self):
        ...
        # 输入vm名
        self.type_text('ding-pu', *self.vm_name)
        # 点击已保存密码库选项
    ...
    def advanced_set(self):
        ...
        WebDriverWait(self.driver, 50, 0.5).until(EC.presence_of_element_located((self.public_button)))
        self.click_element(*self.public_button)
        ...
        WebDriverWait(self.driver, 2500, 0.5).until(EC.text_to_be_present_in_element((By.XPATH, ("//div[text()='Created 2 virtual machines with base name ding-pu']")), 'Created 2 virtual machines with base name ding-pu'))
        refresh = self.locate_elements(*self.refresh_button)
        refresh[1].click()
        WebDriverWait(self.driver, 30, 0.5).until(EC.presence_of_element_located((self.pu_vm1)))
        self.click_element(*self.pu_vm1)
    ...
    #     pyautogui.click(850,475, duration=2)
        # sleep(3)
        # pyautogui.click(1420,140, duration=2)# 删除

    def view_resource(self):
        sleep(5)
        # ess = self.locate_elements(*self.essentials)
        # ess[1].click()
        # sleep(2)
        ...
        fork2[1].click()
        # 点击第二个pu
        WebDriverWait(self.driver, 30, 0.5).until(EC.presence_of_element_located((self.pu_vm2)))
        self.click_element(*self.pu_vm2)
        sleep(5)
        fork2 = self.locate_elements(*self.close_button)
        fork2[1].click()
        sleep(2)
        ...
        WebDriverWait(self.driver, 1000, 0.5).until(EC.text_to_be_present_in_element((By.XPATH, ("//div[text()='Deleted Virtual machine ding-pu000']")), 'Deleted Virtual machine ding-pu000'))
        refresh = self.locate_elements(*self.refresh_button)
        refresh[1].click()
        sleep(1)
        try:
            assert self.locate_element(*self.pu_vm2).is_displayed()==False
            print("test pass")
